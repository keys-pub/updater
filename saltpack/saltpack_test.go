...
func TestVerify(t *testing.T) {
	...
	err := VerifyDetached(reader, signature1, validCodeSigningKIDs)
	assert.NoError(t, err)
}

func TestVerifyDetachedFileAtPath(t *testing.T) {
	err := VerifyDetachedFileAtPath(testZipPath, testZipSignature, validCodeSigningKIDs)
	assert.NoError(t, err)
}

func TestVerifyFail(t *testing.T) {
	invalid := bytes.NewReader([]byte("This is a test message changed\n"))
	err := VerifyDetached(invalid, signature1, validCodeSigningKIDs)
	require.EqualError(t, err, "invalid signature")
}

func TestVerifyFailDetachedFileAtPath(t *testing.T) {
	err := VerifyDetachedFileAtPath(testZipPath, testZipSignature, map[string]bool{})
	require.Error(t, err)
}

func TestVerifyNoValidIDs(t *testing.T) {
	reader := bytes.NewReader([]byte(message1))
	err := VerifyDetached(reader, signature1, nil)
...
func TestVerifyBadValidIDs(t *testing.T) {
	...
	reader := bytes.NewReader([]byte(message1))
	err := VerifyDetached(reader, signature1, badCodeSigningKIDs)
	require.EqualError(t, err, "Unknown signer KID: 9092ae4e790763dc7343851b977930f35b16cf43ab0ad900a2af3d3ad5cea1a1")
}

func TestVerifyNilInput(t *testing.T) {
	err := VerifyDetached(nil, signature1, validCodeSigningKIDs)
	require.EqualError(t, err, "No reader")
}

func TestVerifyNoSignature(t *testing.T) {
	reader := bytes.NewReader([]byte(message1))
	err := VerifyDetached(reader, "", validCodeSigningKIDs)
...
func TestCheckNilSender(t *testing.T) {
	err := checkSender(nil, validCodeSigningKIDs)
	require.Error(t, err)
}

func TestCheckNoKID(t *testing.T) {
	err := checkSender(testSigningKey{kid: nil}, validCodeSigningKIDs)
	require.Error(t, err)
}

func TestVerifyNoFile(t *testing.T) {
	err := VerifyDetachedFileAtPath("/invalid", signature1, validCodeSigningKIDs)
	assert.Error(t, err)
